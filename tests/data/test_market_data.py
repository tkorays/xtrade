...
def _sample_bars(symbol: str, days: int = 5) -> pd.DataFrame:
        ...
        rows.append(
            {
                ...
                "pre_close": (Decimal("10") + i - 1) if i > 0 else None,
            }
        )
...
def test_kline_upsert_idempotent(engine, schema) -> None:
    ...
    repo.upsert_bars(_sample_bars("AAA"))
    repo.upsert_bars(_sample_bars("AAA"))
    assert repo.count(symbol="AAA") == 5
